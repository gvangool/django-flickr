...
class PhotoManager(models.Manager):
    ...
    def _prepare_data(
        self, photo, flickr_user, info=None, exif=None, geo=None, **kwargs
    ):
        """
        ...
        if info:
            ...
            photo_info = {
                ...
                "date_taken": "%s%s"
                % (info_bunch.dates.taken, flickr_user.tzoffset or ""),
                "date_taken_granularity": info_bunch.dates.takengranularity,
                "date_updated": ts_to_dt(
                    info_bunch.dates.lastupdate, flickr_user.tzoffset
                ),
            ...
            photo_info = {
                ...
                "description": getattr(
                    getattr(photo_bunch, "description", {}), "_content", ""
                ),
                "date_posted": ts_to_dt(
                    getattr(photo_bunch, "dateupload", ""), flickr_user.tzoffset
                ),
                "date_taken": getattr(photo_bunch, "datetaken", ""),
                "date_taken_granularity": getattr(
                    photo_bunch, "datetakengranularity", ""
                ),
                "date_updated": ts_to_dt(
                    getattr(photo_bunch, "lastupdate", ""), flickr_user.tzoffset
                ),
            ...
            if photo_info["date_taken"]:
                photo_info["date_taken"] = "%s%s" % (
                    photo_info["date_taken"],
                    flickr_user.tzoffset or "",
                )
    ...
    def create_or_update_from_json(
        self, flickr_user, info, sizes=None, exif=None, geo=None, **kwargs
    ):
...
class Photo(FlickrModel):
    ...
    def get_next_by_date_posted(self):
        try:
            return (
                Photo.objects.filter(date_posted__gte=self.date_posted)
                .exclude(flickr_id=self.flickr_id)
                .order_by("date_posted", "date_taken")[:1]
                .get()
            )
        except:
            pass
    ...
    def get_next_public_by_date_posted(self):
        try:
            return (
                ...
                .filter(date_posted__gte=self.date_posted)
                .exclude(flickr_id=self.flickr_id)
                .order_by("date_posted", "date_taken")[:1]
                .get()
            )
    ...
    def get_previous_by_date_posted(self):
        try:
            return (
                Photo.objects.filter(date_posted__lte=self.date_posted)
                .exclude(flickr_id=self.flickr_id)
                .order_by("-date_posted", "-date_taken")[:1]
                .get()
            )
    ...
    def get_previous_public_by_date_posted(self):
        try:
            return (
                Photo.objects.public()
                .filter(date_posted__lte=self.date_posted)
                .exclude(flickr_id=self.flickr_id)
                .order_by("-date_posted", "-date_taken")[:1]
                .get()
            )
...
class PhotoSize(object):
    ...
    def __init__(self, photo, **kwargs):
        ...
        self.format = (
            "jpg" if not self.format_field else getattr(self.photo, self.format_field)
        )
    ...
    def _get_source(self):
        if not self._source:
            ...
            if not self._source:
                self._source = build_photo_source(
                    self.photo.farm,
                    self.photo.server,
    ...
    def _get_url(self):
        if not self._url:
            ...
            if not self._url:
                self._url = "%s%s/sizes/%s/" % (
                    self.photo.user.flickr_page_url,
                    self.photo.flickr_id,
                    self.url_suffix,
                )
        return self._url
...
for key, size in list(FLICKR_PHOTO_SIZES.items()):
    ...
    for dato in ["source", "url", "width", "height"]:
        ...
        def get_property(self, label=label, dato=dato):
            ...
            string = (
                "Accessing photo sizes properties through '%s' is deprecated. Use '%s' instead."
                % (method_deprecated, method_suggested)
            )
            warn(string)
...
class PhotoSetManager(models.Manager):
    ...
    def _prepare_data(self, info, photos, flickr_user, exif=None, geo=None):
        ...
        data = {
            ...
            "date_posted": ts_to_dt(photoset.date_create, flickr_user.tzoffset),
            "date_updated": ts_to_dt(photoset.date_update, flickr_user.tzoffset),
            "photos": photos,
            "last_sync": now(),
        }
        if flickr_user:
    ...
    def update_from_json(
        self, flickr_user, flickr_id, info, photos, update_photos=False, **kwargs
    ):
        """Update a record with flickr_id"""
        photoset_data = self._prepare_data(
            info=info, photos=photos, flickr_user=flickr_user, **kwargs
        )
...
class Collection(FlickrModel):
    ...
    @property
    def flickr_page_url(self):
        return "%scollections/%s/" % (
            self.user.flickr_page_url,
            (self.flickr_id.split("-")[-1]),
        )
...
def upload_path(obj, filename):
    ...
    return "/".join(
        [dirbase, str(obj.photo.date_posted.date().strftime(dirformat)), filename]
    )
